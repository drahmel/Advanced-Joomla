...
class Form_builderViewform_builder
{
	function setForm_builderToolbar()
	{
		JToolBarHelper::title( JText::_( 'Form_builder Manager' ), 'generic.png' );
  ...
  function form_builder( &$rows, &$pageNav, &$lists )
  {
	FormBuilderViewFormbuilder::setForm_builderToolbar();
	$user =& JFactory::getUser();
	JHTML::_('behavior.tooltip');
	?>
	<form action="index.php?option=com_FormBuilder" method="post" name="adminForm">
	<table>
	<tr>
	  ...
	  <thead>
		<tr>
		  ...
		  <th width="5%" nowrap="nowrap">
			<?php echo JHTML::_('grid.sort',   'Published', 'b.showFormbuilder', @$lists['order_Dir'], @$lists['order'] ); ?>
		  </th>
	  ...
	  for ($i=0, $n=count( $rows ); $i < $n; $i++) {
		...
		$row->id  = $row->bid;
		$link	 = JRoute::_( 'index.php?option=com_FormBuilder&task=edit&cid[]='. $row->id );
		...
		$row->published = $row->showFormbuilder;
	...
	<input type="hidden" name="c" value="formbuilder" />
	<input type="hidden" name="option" value="com_FormBuilder" />
  ...
  function setFormbuilderToolbar()
  {
	$task = JRequest::getVar( 'task', '', 'method', 'string');

	JToolBarHelper::title( $task == 'add' ? JText::_( 'Formbuilder' ) . ': <small><small>[ '. JText::_( 'New' ) .' ]</small></small>' : JText::_( 'Formbuilder' ) . ': <small><small>[ '. JText::_( 'Edit' ) .' ]</small></small>', 'generic.png' );
	JToolBarHelper::save( 'save' );
	JToolBarHelper::apply('apply');
	JToolBarHelper::cancel( 'cancel' );
	JToolBarHelper::help( 'screen.FormBuilder.edit' );
  }

  function formbuilder( &$row, &$lists )
  {
	FormBuilderViewFormbuilder::setFormbuilderToolbar();
	JRequest::setVar( 'hidemainmenu', 1 );
	JFilterOutput::objectHTMLSafe( $row, ENT_QUOTES, 'customformbuildercode' );
	?>
	<script language="javascript" type="text/javascript">
	<!--
	function changeDisplayImage() {
	  if (document.adminForm.imageurl.value !='') {
		document.adminForm.imagelib.src='../images/FormBuilder/' + document.adminForm.imageurl.value;
	  } else {
	...
	function submitbutton(pressbutton) {
	  ...
	  // do field validation
	  if (form.name.value == "") {
		alert( "<?php echo JText::_( 'You must provide a formbuilder name.', true ); ?>" );
	  ...
	  /*} else if (form.clickurl.value == "") {
		alert( "<?php echo JText::_( 'Please fill in the URL for the formbuilder.', true ); ?>" );*/
	...
	<div class="col100">
	  <fieldset class="adminform">
		...
		<tbody>
		  ...
		  <tr>
			<td class="key">
			  <?php echo JText::_( 'Show Formbuilder' ); ?>:
			</td>
			<td>
			  <?php echo $lists['showFormbuilder']; ?>
			</td>
		  </tr>
		  <tr>
			<td class="key">
		  ...
		  <tr>
			<td valign="top" class="key">
			  <label for="customformbuildercode">
				<?php echo JText::_( 'Custom formbuilder code' ); ?>:
			  </label>
			</td>
			<td>
			  <textarea class="inputbox" cols="70" rows="8" name="customformbuildercode" id="customformbuildercode"><?php echo $row->customformbuildercode;?></textarea>
			</td>
		  </tr>
		  <tr>
			<td valign="top" class="key">
		  ...
		  <tr>
			<td valign="top" class="key">
			  <label for="imageurl">
				<?php echo JText::_( 'Formbuilder Image Selector' ); ?>:
			  </label>
			</td>
			<td >
		  ...
		  <tr>
			<td valign="top" class="key">
			  <?php echo JText::_( 'Formbuilder Image' ); ?>:
			</td>
			<td valign="top">
			  <?php
			  if (eregi("swf", $row->imageurl)) {
				...
				?>
				<img src="../images/FormBuilder/<?php echo $row->imageurl; ?>" name="imagelib" />
				<?php
			  } else {
				?>
				<img src="images/blank.png" name="imagelib" />
